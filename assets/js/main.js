...
// const bodyPopUp = document.getElementById('body-pop-up');
...
const jsFormSend = document.querySelector('.js-form-send');
const jsFormButton = document.querySelector('.js-form-button');

if (jsFormSend) {
  jsFormSend.addEventListener('submit', function(e) {
      e.preventDefault();
      ajax({
...
function ajax(o) {
  var type = o.type || 'POST',
      ...
      callback_err = o.callback_err || o.callback,
      x = new XMLHttpRequest();
  if(headers == null && (typeof o.data === 'string')) {
      headers = {};
      headers['Content-type'] = 'application/x-www-form-urlencoded';
  }
  x.open(type, o.url, true);
  for(i in headers) {
  ...
  x.onreadystatechange = function() {
      ...
      clearTimeout(t);
      if(x.status == 200) {
          o.callback(x.responseText);
      } else {
          callback_err('error: ' + x.statusText);
      }
  }
  x.send(o.data);
  var t = setTimeout(function() { x.abort(); callback_err('error: timeout') }, 20000);
  return x;
}
...
const izometriyaL = document.querySelector('[data-l="izometriya"]');
const izometriyaLBtn = document.querySelector('.sochi__nav-izometriya-l');

const planL = document.querySelector('[data-l="plan"]');
const planLBtn = document.querySelector('.sochi__nav-plan-l');

const squareL = document.querySelector('[data-l="square"]');
const squareLBtn = document.querySelector('.sochi__nav-square-l');
...
/* izometriyaL */ 
if (izometriyaL) {
  izometriyaLBtn.classList.add('draft-text-color-active')
  getIzometriyaL();

  izometriyaLBtn.addEventListener('click', function() {
    izometriyaL.classList.remove('none');
    izometriyaLBtn.classList.add('draft-text-color-active')

    planL.classList.add('none');
    planLBtn.classList.remove('draft-text-color-active')

    squareL.classList.add('none');
    squareLBtn.classList.remove('draft-text-color-active')
...
function getIzometriyaL() {
  const izometriyaLList = Array.from(izometriyaL.children);
  const levelWrapperSochi = document.querySelector('.level-wrapper-sochi');

  // Удаляем все этажи
  if (levelWrapperSochi.children.length > 0) {
    levelWrapperSochi.innerHTML = '';
  }

  // Создаем этажи
  for (let i = izometriyaLList.length - 1; i >= 0; i--) {
    const floor = document.createElement('div');
    floor.textContent = (i + 1) + ' Этаж';
    floor.classList.add('level-item');
    floor.classList.add('level-standart');
    levelWrapperSochi.appendChild(floor);
  }
  levelWrapperSochi.lastChild.classList.add('color-active');

  // Задаем data-atribut для картинок и скрываем все кроме последнего элемента
  izometriyaLList.forEach(function(izometriyaItem, index) {
    izometriyaItem.dataset.indexIzometriyaL = index;
    if (izometriyaItem.dataset.indexIzometriyaL != (izometriyaLList.length - 1)) {
      izometriyaItem.classList.add('none');
    } else {
      izometriyaItem.classList.remove('none');
    }
  })

  // Задаем data-atribut для этажей
  Array.from(levelWrapperSochi.children).forEach(function(izometriyaLevelL, izometriyaIndexL) {
    izometriyaLevelL.dataset.izometriyaIndexL = izometriyaIndexL;
   
    izometriyaLevelL.addEventListener('click', function() {

      for (let i = 0; i < izometriyaLList.length; i++) {
        izometriyaLList[i].classList.add('none');
        Array.from(levelWrapperSochi.children)[i].classList.remove('color-active');
      }

      const elem = document.querySelector(`[data-index-izometriya-l="${izometriyaLevelL.dataset.izometriyaIndexL}"]`)
      elem.classList.remove('none');
      izometriyaLevelL.classList.add('color-active');
...
if (planL) {
  ...
  planLBtn.addEventListener('click', function() {

    izometriyaL.classList.add('none');
    izometriyaLBtn.classList.remove('draft-text-color-active')

    planL.classList.remove('none');
    planLBtn.classList.add('draft-text-color-active')
...
function getPlanL() {
  const planLList = Array.from(planL.children);
  const levelWrapperSochi = document.querySelector('.level-wrapper-sochi');
  ...
  // Создаем этажи
  for (let i = planLList.length - 1; i >= 0; i--) {
    const floor = document.createElement('div');
    floor.textContent = (i + 1) + ' Этаж';
    floor.classList.add('level-item');
    floor.classList.add('level-standart');
  ...
  // Задаем data-atribut для картинок и скрываем все кроме последнего элемента
  planLList.forEach(function(planItem, index) {
    planItem.dataset.indexPlanL = index;
    if (planItem.dataset.indexPlanL != (planLList.length - 1)) {
      planItem.classList.add('none');
    } else {
  ...
  // Задаем data-atribut для этажей
  Array.from(levelWrapperSochi.children).forEach(function(planLevelL, planIndexL) {
    planLevelL.dataset.planIndexL = planIndexL;
   
    planLevelL.addEventListener('click', function() {

      for (let i = 0; i < planLList.length; i++) {
        planLList[i].classList.add('none');
        Array.from(levelWrapperSochi.children)[i].classList.remove('color-active');
      }

      const elem = document.querySelector(`[data-index-plan-l="${planLevelL.dataset.planIndexL}"]`)
      elem.classList.remove('none');
      planLevelL.classList.add('color-active');
...
if (squareL) {
  ...
  squareLBtn.addEventListener('click', function() {
    ...
    squareL.classList.remove('none');
    squareLBtn.classList.add('draft-text-color-active')
...
function getSquareL() {
  const squareLList = Array.from(squareL.children);
  const levelWrapperSochi = document.querySelector('.level-wrapper-sochi');
  ...
  // Создаем этажи
  for (let i = squareLList.length - 1; i >= 0; i--) {
    const floor = document.createElement('div');
    floor.textContent = (i + 1) + ' Этаж';
    floor.classList.add('level-item');
    floor.classList.add('level-standart');
  ...
  // Задаем data-atribut для картинок и скрываем все кроме последнего элемента
  squareLList.forEach(function(squareItem, index) {
    squareItem.dataset.indexSquareL = index;
    if (squareItem.dataset.indexSquareL != (squareLList.length - 1)) {
      squareItem.classList.add('none');
    } else {
      squareItem.classList.remove('none');
    }
  })

  // Задаем data-atribut для этажей
  Array.from(levelWrapperSochi.children).forEach(function(squareLevelL, squareIndexL) {
    squareLevelL.dataset.squareIndexL = squareIndexL;
   
    squareLevelL.addEventListener('click', function() {

      for (let i = 0; i < squareLList.length; i++) {
        squareLList[i].classList.add('none');
        Array.from(levelWrapperSochi.children)[i].classList.remove('color-active');
      }

      const elem = document.querySelector(`[data-index-square-l="${squareLevelL.dataset.squareIndexL}"]`)
      elem.classList.remove('none');
      squareLevelL.classList.add('color-active');
...
const izometriyaM = document.querySelector('[data-m="izometriya"]');
const izometriyaMBtn = document.querySelector('.sochi__nav-izometriya-m');

const planM = document.querySelector('[data-m="plan"]');
const planMBtn = document.querySelector('.sochi__nav-plan-m');

const squareM = document.querySelector('[data-m="square"]');
const squareMBtn = document.querySelector('.sochi__nav-square-m');
...
if (izometriyaM) {
  ...
  izometriyaMBtn.classList.add('draft-text-color-active')
  getIzometriyaM();

  izometriyaMBtn.addEventListener('click', function() {
    izometriyaM.classList.remove('none');
    izometriyaMBtn.classList.add('draft-text-color-active')

    planM.classList.add('none');
    planMBtn.classList.remove('draft-text-color-active')

    squareM.classList.add('none');
    squareMBtn.classList.remove('draft-text-color-active')
...
function getIzometriyaM() {
  const izometriyaMList = Array.from(izometriyaM.children);
  const levelWrapperSochi = document.querySelector('.level-wrapper-sochi-m');
  ...
  // Создаем этажи
  for (let i = izometriyaMList.length - 1; i >= 0; i--) {
    const floor = document.createElement('div');
    floor.textContent = (i + 1) + ' Этаж';
    floor.classList.add('level-item');
    floor.classList.add('level-standart');
  ...
  // Задаем data-atribut для картинок и скрываем все кроме последнего элемента
  izometriyaMList.forEach(function(izometriyaItem, index) {
    izometriyaItem.dataset.indexIzometriyaM = index;
    if (izometriyaItem.dataset.indexIzometriyaM != (izometriyaMList.length - 1)) {
      izometriyaItem.classList.add('none');
    } else {
      izometriyaItem.classList.remove('none');
    }
  ...
  // Задаем data-atribut для этажей
  Array.from(levelWrapperSochi.children).forEach(function(izometriyaLevelM, izometriyaIndexM) {
    izometriyaLevelM.dataset.izometriyaIndexM = izometriyaIndexM;
   
    izometriyaLevelM.addEventListener('click', function() {

      for (let i = 0; i < izometriyaMList.length; i++) {
        izometriyaMList[i].classList.add('none');
        Array.from(levelWrapperSochi.children)[i].classList.remove('color-active');
      }

      const elem = document.querySelector(`[data-index-izometriya-m="${izometriyaLevelM.dataset.izometriyaIndexM}"]`)
      elem.classList.remove('none');
      izometriyaLevelM.classList.add('color-active');
...
if (planM) {
  ...
  planMBtn.addEventListener('click', function() {

    izometriyaM.classList.add('none');
    izometriyaMBtn.classList.remove('draft-text-color-active')

    planM.classList.remove('none');
    planMBtn.classList.add('draft-text-color-active')
...
function getPlanM() {
  const planMList = Array.from(planM.children);
  const levelWrapperSochi = document.querySelector('.level-wrapper-sochi-m');
  ...
  // Создаем этажи
  for (let i = planMList.length - 1; i >= 0; i--) {
    const floor = document.createElement('div');
    floor.textContent = (i + 1) + ' Этаж';
    floor.classList.add('level-item');
    floor.classList.add('level-standart');
  ...
  // Задаем data-atribut для картинок и скрываем все кроме последнего элемента
  planMList.forEach(function(planItem, index) {
    planItem.dataset.indexPlanM = index;
    if (planItem.dataset.indexPlanM != (planMList.length - 1)) {
      planItem.classList.add('none');
    } else {
      planItem.classList.remove('none');
    }
  ...
  // Задаем data-atribut для этажей
  Array.from(levelWrapperSochi.children).forEach(function(planLevelM, planIndexM) {
    planLevelM.dataset.planIndexM = planIndexM;
   
    planLevelM.addEventListener('click', function() {

      for (let i = 0; i < planMList.length; i++) {
        planMList[i].classList.add('none');
        Array.from(levelWrapperSochi.children)[i].classList.remove('color-active');
      }

      const elem = document.querySelector(`[data-index-plan-m="${planLevelM.dataset.planIndexM}"]`)
      elem.classList.remove('none');
      planLevelM.classList.add('color-active');
...
if (squareM) {
  ...
  squareMBtn.addEventListener('click', function() {
    ...
    squareM.classList.remove('none');
    squareMBtn.classList.add('draft-text-color-active')
...
function getSquareM() {
  const squareMList = Array.from(squareM.children);
  const levelWrapperSochi = document.querySelector('.level-wrapper-sochi-m');
  ...
  // Создаем этажи
  for (let i = squareMList.length - 1; i >= 0; i--) {
    const floor = document.createElement('div');
    floor.textContent = (i + 1) + ' Этаж';
    floor.classList.add('level-item');
    floor.classList.add('level-standart');
  ...
  // Задаем data-atribut для картинок и скрываем все кроме последнего элемента
  squareMList.forEach(function(squareItem, index) {
    squareItem.dataset.indexSquareM = index;
    if (squareItem.dataset.indexSquareM != (squareMList.length - 1)) {
      squareItem.classList.add('none');
    } else {
      squareItem.classList.remove('none');
    }
  ...
  // Задаем data-atribut для этажей
  Array.from(levelWrapperSochi.children).forEach(function(squareLevelM, squareIndexM) {
    squareLevelM.dataset.squareIndexM = squareIndexM;
   
    squareLevelM.addEventListener('click', function() {

      for (let i = 0; i < squareMList.length; i++) {
        squareMList[i].classList.add('none');
        Array.from(levelWrapperSochi.children)[i].classList.remove('color-active');
      }

      const elem = document.querySelector(`[data-index-square-m="${squareLevelM.dataset.squareIndexM}"]`)
      elem.classList.remove('none');
      squareLevelM.classList.add('color-active');
...
const izometriyaS = document.querySelector('[data-s="izometriya"]');
const izometriyaSBtn = document.querySelector('.sochi__nav-izometriya-s');

const planS = document.querySelector('[data-s="plan"]');
const planSBtn = document.querySelector('.sochi__nav-plan-s');

const squareS = document.querySelector('[data-s="square"]');
const squareSBtn = document.querySelector('.sochi__nav-square-s');
...
/* izometriyaS */ 
if (izometriyaS) {
  izometriyaSBtn.classList.add('draft-text-color-active')
  getIzometriyaS();

  izometriyaSBtn.addEventListener('click', function() {
    izometriyaS.classList.remove('none');
    izometriyaSBtn.classList.add('draft-text-color-active')

    planS.classList.add('none');
    planSBtn.classList.remove('draft-text-color-active')

    squareS.classList.add('none');
    squareSBtn.classList.remove('draft-text-color-active')
...
function getIzometriyaS() {
  const izometriyaSList = Array.from(izometriyaS.children);
  const levelWrapperSochi = document.querySelector('.level-wrapper-sochi-s');
  ...
  // Создаем этажи
  for (let i = izometriyaSList.length - 1; i >= 0; i--) {
    const floor = document.createElement('div');
    floor.textContent = (i + 1) + ' Этаж';
    floor.classList.add('level-item');
    floor.classList.add('level-standart');
  ...
  // Задаем data-atribut для картинок и скрываем все кроме последнего элемента
  izometriyaSList.forEach(function(izometriyaItem, index) {
    izometriyaItem.dataset.indexIzometriyaS = index;
    if (izometriyaItem.dataset.indexIzometriyaS != (izometriyaSList.length - 1)) {
      izometriyaItem.classList.add('none');
    } else {
      izometriyaItem.classList.remove('none');
    }
  ...
  // Задаем data-atribut для этажей
  Array.from(levelWrapperSochi.children).forEach(function(izometriyaLevelS, izometriyaIndexS) {
    izometriyaLevelS.dataset.izometriyaIndexS = izometriyaIndexS;
   
    izometriyaLevelS.addEventListener('click', function() {

      for (let i = 0; i < izometriyaSList.length; i++) {
        izometriyaSList[i].classList.add('none');
        Array.from(levelWrapperSochi.children)[i].classList.remove('color-active');
      }

      const elem = document.querySelector(`[data-index-izometriya-s="${izometriyaLevelS.dataset.izometriyaIndexS}"]`)
      elem.classList.remove('none');
      izometriyaLevelS.classList.add('color-active');
...
if (planS) {
  ...
  planSBtn.addEventListener('click', function() {
    ...
    izometriyaS.classList.add('none');
    izometriyaSBtn.classList.remove('draft-text-color-active')

    planS.classList.remove('none');
    planSBtn.classList.add('draft-text-color-active')
...
function getPlanS() {
  const planSList = Array.from(planS.children);
  const levelWrapperSochi = document.querySelector('.level-wrapper-sochi-s');
  ...
  // Задаем data-atribut для картинок и скрываем все кроме последнего элемента
  planSList.forEach(function(planItem, index) {
    planItem.dataset.indexPlanS = index;
    if (planItem.dataset.indexPlanS != (planSList.length - 1)) {
      planItem.classList.add('none');
    } else {
      planItem.classList.remove('none');
    }
  ...
  // Задаем data-atribut для этажей
  Array.from(levelWrapperSochi.children).forEach(function(planLevelS, planIndexS) {
    planLevelS.dataset.planIndexS = planIndexS;
   
    planLevelS.addEventListener('click', function() {

      for (let i = 0; i < planSList.length; i++) {
        planSList[i].classList.add('none');
        Array.from(levelWrapperSochi.children)[i].classList.remove('color-active');
      }

      const elem = document.querySelector(`[data-index-plan-s="${planLevelS.dataset.planIndexS}"]`)
      elem.classList.remove('none');
      planLevelS.classList.add('color-active');
...
if (squareS) {
  ...
  squareSBtn.addEventListener('click', function() {
    ...
    squareS.classList.remove('none');
    squareSBtn.classList.add('draft-text-color-active')
...
function getSquareS() {
  const squareSList = Array.from(squareS.children);
  const levelWrapperSochi = document.querySelector('.level-wrapper-sochi-s');
  ...
  // Создаем этажи
  for (let i = squareSList.length - 1; i >= 0; i--) {
    const floor = document.createElement('div');
    floor.textContent = (i + 1) + ' Этаж';
    floor.classList.add('level-item');
    floor.classList.add('level-standart');
  ...
  // Задаем data-atribut для картинок и скрываем все кроме последнего элемента
  squareSList.forEach(function(squareItem, index) {
    squareItem.dataset.indexSquareS = index;
    if (squareItem.dataset.indexSquareS != (squareSList.length - 1)) {
      squareItem.classList.add('none');
    } else {
      squareItem.classList.remove('none');
    }
  ...
  // Задаем data-atribut для этажей
  Array.from(levelWrapperSochi.children).forEach(function(squareLevelS, squareIndexS) {
    squareLevelS.dataset.squareIndexS = squareIndexS;
   
    squareLevelS.addEventListener('click', function() {

      for (let i = 0; i < squareSList.length; i++) {
        squareSList[i].classList.add('none');
        Array.from(levelWrapperSochi.children)[i].classList.remove('color-active');
      }

      const elem = document.querySelector(`[data-index-square-s="${squareLevelS.dataset.squareIndexS}"]`)
      elem.classList.remove('none');
      squareLevelS.classList.add('color-active');
...
const izometriyaSSS = document.querySelector('[data-sss="izometriya"]');
const izometriyaSSSBtn = document.querySelector('.sochi__nav-izometriya-sss');

const planSSS = document.querySelector('[data-sss="plan"]');
const planSSSBtn = document.querySelector('.sochi__nav-plan-sss');

const squareSSS = document.querySelector('[data-sss="square"]');
const squareSSSBtn = document.querySelector('.sochi__nav-square-sss');
...
/* izometriyaSSS */ 
if (izometriyaSSS) {
  izometriyaSSSBtn.classList.add('draft-text-color-active')
  getIzometriyaSSS();

  izometriyaSSSBtn.addEventListener('click', function() {
    izometriyaSSS.classList.remove('none');
    izometriyaSSSBtn.classList.add('draft-text-color-active')

    planSSS.classList.add('none');
    planSSSBtn.classList.remove('draft-text-color-active')

    squareSSS.classList.add('none');
    squareSSSBtn.classList.remove('draft-text-color-active')
...
function getIzometriyaSSS() {
  const izometriyaSSSList = Array.from(izometriyaSSS.children);
  const levelWrapperSochi = document.querySelector('.level-wrapper-sochi-sss');
  ...
  // Создаем этажи
  for (let i = izometriyaSSSList.length - 1; i >= 0; i--) {
    const floor = document.createElement('div');
    floor.textContent = (i + 1) + ' Этаж';
    floor.classList.add('level-item');
    floor.classList.add('level-standart');
  ...
  // Задаем data-atribut для картинок и скрываем все кроме последнего элемента
  izometriyaSSSList.forEach(function(izometriyaItem, index) {
    izometriyaItem.dataset.indexIzometriyaSSS = index;
    if (izometriyaItem.dataset.indexIzometriyaSSS != (izometriyaSSSList.length - 1)) {
      izometriyaItem.classList.add('none');
    } else {
      izometriyaItem.classList.remove('none');
    }
  ...
  // Задаем data-atribut для этажей
  Array.from(levelWrapperSochi.children).forEach(function(izometriyaLevelSSS, izometriyaIndexSSS) {
    izometriyaLevelSSS.dataset.izometriyaIndexSSS = izometriyaIndexSSS;
   
    izometriyaLevelSSS.addEventListener('click', function() {

      for (let i = 0; i < izometriyaSSSList.length; i++) {
        izometriyaSSSList[i].classList.add('none');
        Array.from(levelWrapperSochi.children)[i].classList.remove('color-active');
      }

      const elem = document.querySelector(`[data-index-izometriya-s-s-s="${izometriyaLevelSSS.dataset.izometriyaIndexSSS}"]`)
      elem.classList.remove('none');
      izometriyaLevelSSS.classList.add('color-active');
...
if (planSSS) {
  ...
  planSSSBtn.addEventListener('click', function() {

    izometriyaSSS.classList.add('none');
    izometriyaSSSBtn.classList.remove('draft-text-color-active')

    planSSS.classList.remove('none');
    planSSSBtn.classList.add('draft-text-color-active')
...
function getPlanSSS() {
  const planSSSList = Array.from(planSSS.children);
  const levelWrapperSochi = document.querySelector('.level-wrapper-sochi-sss');
  ...
  // Создаем этажи
  for (let i = planSSSList.length - 1; i >= 0; i--) {
    const floor = document.createElement('div');
    floor.textContent = (i + 1) + ' Этаж';
    floor.classList.add('level-item');
    floor.classList.add('level-standart');
  ...
  // Задаем data-atribut для картинок и скрываем все кроме последнего элемента
  planSSSList.forEach(function(planItem, index) {
    planItem.dataset.indexPlanSSS = index;
    if (planItem.dataset.indexPlanSSS != (planSSSList.length - 1)) {
      planItem.classList.add('none');
    } else {
      planItem.classList.remove('none');
    }
  ...
  // Задаем data-atribut для этажей
  Array.from(levelWrapperSochi.children).forEach(function(planLevelSSS, planIndexSSS) {
    planLevelSSS.dataset.planIndexSSS = planIndexSSS;
   
    planLevelSSS.addEventListener('click', function() {

      for (let i = 0; i < planSSSList.length; i++) {
        planSSSList[i].classList.add('none');
        Array.from(levelWrapperSochi.children)[i].classList.remove('color-active');
      }

      const elem = document.querySelector(`[data-index-plan-s-s-s="${planLevelSSS.dataset.planIndexSSS}"]`)
      elem.classList.remove('none');
      planLevelSSS.classList.add('color-active');
...
if (squareSSS) {
  ...
  console.log(squareSSSBtn)

  squareSSSBtn.addEventListener('click', function() {
    ...
    squareSSS.classList.remove('none');
    squareSSSBtn.classList.add('draft-text-color-active')
...
function getSquareSSS() {
  const squareSSSList = Array.from(squareSSS.children);
  const levelWrapperSochi = document.querySelector('.level-wrapper-sochi-sss');
  ...
  // Создаем этажи
  for (let i = squareSSSList.length - 1; i >= 0; i--) {
    const floor = document.createElement('div');
    floor.textContent = (i + 1) + ' Этаж';
    floor.classList.add('level-item');
    floor.classList.add('level-standart');
  ...
  // Задаем data-atribut для картинок и скрываем все кроме последнего элемента
  squareSSSList.forEach(function(squareItem, index) {
    squareItem.dataset.indexSquareSSS = index;
    if (squareItem.dataset.indexSquareSSS != (squareSSSList.length - 1)) {
      squareItem.classList.add('none');
    } else {
      squareItem.classList.remove('none');
    }
  ...
  // Задаем data-atribut для этажей
  Array.from(levelWrapperSochi.children).forEach(function(squareLevelSSS, squareIndexSSS) {
    squareLevelSSS.dataset.squareIndexSSS = squareIndexSSS;
   
    squareLevelSSS.addEventListener('click', function() {

      for (let i = 0; i < squareSSSList.length; i++) {
        squareSSSList[i].classList.add('none');
        Array.from(levelWrapperSochi.children)[i].classList.remove('color-active');
      }

      const elem = document.querySelector(`[data-index-square-s-s-s="${squareLevelSSS.dataset.squareIndexSSS}"]`)
      elem.classList.remove('none');
      squareLevelSSS.classList.add('color-active');
...
const izometriyaXS = document.querySelector('[data-xs="izometriya"]');
const izometriyaXSBtn = document.querySelector('.sochi__nav-izometriya-xs');

const planXS = document.querySelector('[data-xs="plan"]');
const planXSBtn = document.querySelector('.sochi__nav-plan-xs');

const squareXS = document.querySelector('[data-xs="square"]');
const squareXSBtn = document.querySelector('.sochi__nav-square-xs');
...
/* izometriyaXS */ 
if (izometriyaXS) {
  izometriyaXSBtn.classList.add('draft-text-color-active')
  getIzometriyaXS();

  izometriyaXSBtn.addEventListener('click', function() {
    izometriyaXS.classList.remove('none');
    izometriyaXSBtn.classList.add('draft-text-color-active')

    planXS.classList.add('none');
    planXSBtn.classList.remove('draft-text-color-active')

    squareXS.classList.add('none');
    squareXSBtn.classList.remove('draft-text-color-active')
...
function getIzometriyaXS() {
  const izometriyaXSList = Array.from(izometriyaXS.children);
  const levelWrapperSochi = document.querySelector('.level-wrapper-sochi-xs');
  ...
  // Создаем этажи
  for (let i = izometriyaXSList.length - 1; i >= 0; i--) {
    const floor = document.createElement('div');
    floor.textContent = (i + 1) + ' Этаж';
    floor.classList.add('level-item');
    floor.classList.add('level-standart');
  ...
  // Задаем data-atribut для картинок и скрываем все кроме последнего элемента
  izometriyaXSList.forEach(function(izometriyaItem, index) {
    izometriyaItem.dataset.indexIzometriyaXS = index;
    if (izometriyaItem.dataset.indexIzometriyaXS != (izometriyaXSList.length - 1)) {
      izometriyaItem.classList.add('none');
    } else {
      izometriyaItem.classList.remove('none');
    }
  ...
  // Задаем data-atribut для этажей
  Array.from(levelWrapperSochi.children).forEach(function(izometriyaLevelXS, izometriyaIndexXS) {
    izometriyaLevelXS.dataset.izometriyaIndexXS = izometriyaIndexXS;
   
    izometriyaLevelXS.addEventListener('click', function() {

      for (let i = 0; i < izometriyaXSList.length; i++) {
        izometriyaXSList[i].classList.add('none');
        Array.from(levelWrapperSochi.children)[i].classList.remove('color-active');
      }

      const elem = document.querySelector(`[data-index-izometriya-x-s="${izometriyaLevelXS.dataset.izometriyaIndexXS}"]`)
      elem.classList.remove('none');
      izometriyaLevelXS.classList.add('color-active');
...
if (planXS) {
  ...
  planXSBtn.addEventListener('click', function() {

    izometriyaXS.classList.add('none');
    izometriyaXSBtn.classList.remove('draft-text-color-active')

    planXS.classList.remove('none');
    planXSBtn.classList.add('draft-text-color-active')
...
function getPlanXS() {
  const planXSList = Array.from(planXS.children);
  const levelWrapperSochi = document.querySelector('.level-wrapper-sochi-xs');
  ...
  // Создаем этажи
  for (let i = planXSList.length - 1; i >= 0; i--) {
    const floor = document.createElement('div');
    floor.textContent = (i + 1) + ' Этаж';
    floor.classList.add('level-item');
    floor.classList.add('level-standart');
  ...
  // Задаем data-atribut для картинок и скрываем все кроме последнего элемента
  planXSList.forEach(function(planItem, index) {
    planItem.dataset.indexPlanXS = index;
    if (planItem.dataset.indexPlanXS != (planXSList.length - 1)) {
      planItem.classList.add('none');
    } else {
      planItem.classList.remove('none');
    }
  ...
  // Задаем data-atribut для этажей
  Array.from(levelWrapperSochi.children).forEach(function(planLevelXS, planIndexXS) {
    planLevelXS.dataset.planIndexXS = planIndexXS;
   
    planLevelXS.addEventListener('click', function() {

      for (let i = 0; i < planXSList.length; i++) {
        planXSList[i].classList.add('none');
        Array.from(levelWrapperSochi.children)[i].classList.remove('color-active');
      }

      const elem = document.querySelector(`[data-index-plan-x-s="${planLevelXS.dataset.planIndexXS}"]`)
      elem.classList.remove('none');
      planLevelXS.classList.add('color-active');
...
if (squareXS) {
  ...
  console.log(squareXS)
  console.log(squareXSBtn)

  squareXSBtn.addEventListener('click', function() {
    ...
    squareXS.classList.remove('none');
    squareXSBtn.classList.add('draft-text-color-active')
...
function getSquareXS() {
  const squareXSList = Array.from(squareXS.children);
  const levelWrapperSochi = document.querySelector('.level-wrapper-sochi-xs');
  ...
  // Создаем этажи
  for (let i = squareXSList.length - 1; i >= 0; i--) {
    const floor = document.createElement('div');
    floor.textContent = (i + 1) + ' Этаж';
    floor.classList.add('level-item');
    floor.classList.add('level-standart');
  ...
  // Задаем data-atribut для картинок и скрываем все кроме последнего элемента
  squareXSList.forEach(function(squareItem, index) {
    squareItem.dataset.indexSquareXS = index;
    if (squareItem.dataset.indexSquareXS != (squareXSList.length - 1)) {
      squareItem.classList.add('none');
    } else {
      squareItem.classList.remove('none');
    }
  ...
  // Задаем data-atribut для этажей
  Array.from(levelWrapperSochi.children).forEach(function(squareLevelXS, squareIndexXS) {
    squareLevelXS.dataset.squareIndexXS = squareIndexXS;
   
    squareLevelXS.addEventListener('click', function() {

      for (let i = 0; i < squareXSList.length; i++) {
        squareXSList[i].classList.add('none');
        Array.from(levelWrapperSochi.children)[i].classList.remove('color-active');
      }

      const elem = document.querySelector(`[data-index-square-x-s="${squareLevelXS.dataset.squareIndexXS}"]`)
      elem.classList.remove('none');
      squareLevelXS.classList.add('color-active');
